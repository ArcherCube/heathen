  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          'flex aspect-video h-full w-full justify-center text-xs',
          "[&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/50",
          '[&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-layer]:outline-hidden',
          ...
          "[&_.recharts-sector]:outline-hidden [&_.recharts-surface]:outline-hidden [&_.recharts-dot[stroke='#fff']]:stroke-transparent",
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const html = useCreation(() => {
    return Object.entries(THEMES)
      .map(([theme, prefix]) => {
        const themeConfigStyle = colorConfig
          .map(([key, itemConfig]) => {
            const color = itemConfig.theme?.[theme as keyof typeof itemConfig.theme] ?? itemConfig.color;
            return color ? `  --color-${key}: ${color};` : null;
          })
          .join('\n');
        return `${prefix} [data-chart=${id}] {${themeConfigStyle}}`;
      })
      .join('\n');
  }, [colorConfig]);
